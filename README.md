...
To understand what a `branch` is, try to visualise a git repository as a timeline of changes. Along this timeline, there are `commits` which represent a milestone of changes being saved to the repository. At any point in time, you may create a new `branch` which splits the current timeline into two parallel lines that are maintained separately and receive their own commits.  
